...
int sqlite_update_versionBak_config_db(char *jobid)
{
  ...
  sprintf(tempdata, "update config set softWare_bak='%s',  otaResult='ok' where v2id>=0", ver);//
  debug_print("tempdata = %s \n", tempdata);
  err = sqlite3_exec(config_db, tempdata, NULL, NULL, &zErrMsg);
  sqlite3_close_v2(config_db);
...
int sqlite_update_ota_jobid_config_db(char *jobid)
{
  ...
  sprintf(tempdata, "update config set jobId='%s' where v2id>=0", jobid);//
  debug_print("tempdata = %s \n", tempdata);
  err = sqlite3_exec(config_db, tempdata, NULL, NULL, &zErrMsg);
  sqlite3_close_v2(config_db);
  return err;
...
int sqlite_init_ota_otaResult_config_db(void)
{
  ...
  sprintf(tempdata, "update config set otaResult='err' where v2id>=0");//
  debug_print("tempdata = %s \n", tempdata);
  err = sqlite3_exec(config_db, tempdata, NULL, NULL, &zErrMsg);
  sqlite3_close_v2(config_db);
...
char* sqlite_read_otastatus_config_db(void)
{
  ...
  //
  char *sql = "select otaResult from config";
  err = sqlite3_get_table(config_db, sql, &azResult, &nrow, &ncolumn, &zErrMsg);
  if(err) {
    debug_print("Can't select otaResult: %s\n", sqlite3_errmsg(config_db));
    sqlite3_free_table(azResult);
    sqlite3_free(zErrMsg);
    sqlite3_close_v2(config_db);
    return ;
  }
  memcpy(otaStatus, azResult[1], strlen(azResult[1]));
  debug_print("otaResult = %s\n", otaStatus);
  sqlite3_free_table(azResult);
  sqlite3_free(zErrMsg);
  sqlite3_close_v2(config_db);
  return otaStatus;
