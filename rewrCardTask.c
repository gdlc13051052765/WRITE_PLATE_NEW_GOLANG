...
static rfalTagRes tag_bak[EXAMPLE_RFAL_POLLER_DEVICES];
...
int nfc_write_plate_task(void)
{
  ...
  // 	return;
  //获取是否停止写盘状态
  if(!get_stop_write_palte_status()& writePlataStatus==0)//收到停止写盘命令停止写盘
  {
    //   sndMsg.datalen = 0x03;
    //   sndMsg.cmd = WRPLATE_REMOVE_CMD;
    //   sndMsg.data[0] = 0;
    //   //qt_push_data_to_msg(sndMsg);//发送标签离场指令到发送队列
    //   sprintf(logbuf, "%s", "标签已取走");
  ...
  switch(writePlataStatus)
  {
    ...
    case 1:
      ...
      for(i=0; i<tag_num; i++) 
        ...
        if(!(rfal_write_single_block(tag[i].uid, 2, Write_Data, 4))) {
          ...
          writePlataStatus = 2;//跳转到取走标签状态机
          //备份当前uuid
          memcpy(tag_bak[i].uid,tag[i].uid, 8);
    ...
    case 2: //等待取走标签  
      for(i=0; i<filterNum; i++) 
        ...
        tagnumbak[i] = tag_num;   
        usleep(POLL_LABEL_TIMEST);//延时30ms
		  	debug_print("writePlataStatus = 2 remove total tag number = %d \n", tag_num);
      }
      // if(tag_num >1)
      // {
      //   debug_print("multiple tags exist \n");
      ...
      //for(i=0; i<filterNum; i++) //多次读取标签的个数不一致返回重新读标签
			{
      	if(tagnumbak[i] != tag_num & (tag_num>1))
        {
          debug_print("multiple tags exist \n");
          sprintf(logbuf, "%s", "multiple tags exist");
          write_plate_write_rfid_log(logbuf);
      ...
      if(!tag_num) {//标签已取走
        ...
        if(memcmp(tag_bak[0].uid,tag[0].uid,8))//其他标签存在
        {
          sndMsg.datalen = 0x03;
          sndMsg.cmd = WRPLATE_REMOVE_CMD;
          sndMsg.data[0] = 0;
    ...
    case 3://多标签判断离厂操作
      for(i=0; i<filterNum; i++) 
        ...
        tagnumbak[i] = tag_num;   
        usleep(POLL_LABEL_TIMEST);//延时30ms
		  	debug_print("writePlataStatus =3 remove total tag number = %d \n", tag_num);
      }
      if(!tag_num) {//标签已取走
        sndMsg.datalen = 0x03;
        sndMsg.cmd = WRPLATE_REMOVE_CMD;
